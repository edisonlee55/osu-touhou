...
namespace osu.Game.Rulesets.Touhou.Extensions
{
    public static class CherriesExtensions
    {
        ...
        private static Vector2 getSymmetricalXPosition(Vector2 input) => new Vector2(TouhouPlayfield.BASE_SIZE.X - input.X, input.Y);

        private static Vector2 getSymmetricalYPosition(Vector2 input) => new Vector2(input.X, TouhouPlayfield.BASE_SIZE.Y - input.Y);
        ...
        {
            if (position.X > TouhouPlayfield.BASE_SIZE.X || position.X < 0 || position.Y < 0 || position.Y > TouhouPlayfield.BASE_SIZE.Y)
                return false;
